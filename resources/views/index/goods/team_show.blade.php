...
<div class="user-info">
    <div class="right-info">
        入驻时间 <span style="color: #FD6310">{{(int)((time() - strtotime($data['team']->created_at))/86400 + 1)}}</span> &nbsp;天
        <br>
        在线商品 <span style="color: #FD6310;">{{$data['goodsCount']}}</span> &nbsp;件
    </div>
    <div class="text-center">
        <img src="{{URL::asset('upload/'.$data['avatar'])}}" alt="">
        <br><br>
        <span class="user-name">{{$data['team']->name}}</span> &nbsp;<span class="user-type">{{$data['team']->type == 1 ? '个人' : '团队'}}</span>
    </div>
</div>
<div class="goods-container">
    <div id="goods-list" class="goods-list d-flex flex-wrap justify-content-around">
    @foreach($data['goods_list'] as $goods)
        <div class="goods-item card goods-box" data-id="{{$goods->goods_id}}">
            <div class="card-body">
